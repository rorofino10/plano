...
export class ViewerComponent implements OnInit, AfterViewInit {
  ...
  createEngine(file: string | File | undefined) {
    ...
    this.scene.clearColor = Color3.FromHexString(
      this.toggleComponentService.color
    );
    ...
    this.gizmoManager.onAttachedToMeshObservable.add((evt) => {
      ...
      if (this.selectedMesh) {
        ...
        this.selectedMeshscale = Math.abs(
          this.selectedMesh.scaling.z.toFixed(2) * 100
        );
        this.selectedMeshscale = Math.abs(
          this.selectedMesh.scaling.x.toFixed(2) * 100
        );
        this.selectedMeshscale = Math.abs(
          this.selectedMesh.scaling.y.toFixed(2) * 100
        );
      }
    });
    ...
    this.modelo = SceneLoader.ImportMesh(
      '',
      './assets/planos/',
      file + '.gltf',
      this.scene,
  ...
  addSVG() {
    const svg = MeshBuilder.CreateGround(
      'svg',
      { height: 20, width: 20, subdivisions: 8 },
      this.scene
    );
  ...
  addGround() {
    ...
    this.ground = MeshBuilder.CreateGround(
      'ground',
      { height: 1000, width: 1000, subdivisions: 8 },
      this.scene
    );
